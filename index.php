...
if (isset($_REQUEST['key'])) {
    try {
        $stmt = $pdo->prepare("SELECT login FROM users WHERE reset_key = :key");
        $stmt->bindParam(':key', $_REQUEST['key']);
        $stmt->execute();

        if ($stmt->rowCount() > 0) {
            $user = $stmt->fetch();
            $_SESSION['newpasslogin'] = $user['login'];
            echo <<<FORM
        <!-- Форма для нового пароля -->
        <div class="modal" id="newPasswordModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="newPasswordModalLabel" aria-hidden="true" style="display: block;">
            <div class="modal-dialog modal-md modal-dialog-centered modal-dialog-scrollable">
                <div class="modal-content col-sm-6 col-lg-4">
                    <div class="modal-header">
                        <h1 class="modal-title fs-5" id="newPasswordModalLabel">Восстановление пароля</h1>
                        ...
                        <form id="passwordForm">
                            <div class="row align-items-center mb-3">
                                <div class="form-group">
                                    ...
                                    <input type="password" class="form-control" id="newpassword" name="newpassword" pattern="[a-zA-Z0-9]{8,32}" title="Пароль должен содержать не менее 8 символов" placeholder="Введите новый пароль" required>
                                </div>
                            </div>
                ...
                <div class="row justify-content-center">
                    <div class="col-md-12">
                        <div class="p-3 mb-3 field-bg rounded-3">
                            ...
                            <p>В таких случаях может пригодится
                                <a class="link-success fw-bold" href="/">Word Finder</a>, который поможет отыскать нужные слова по маске или другим параметрам.
                            </p>
                            <h3>Наши услуги</h3>
                            ...
                            <ul>
                                ...
                                <li>Эргономичный интерфейс. В интерфейсе сайта довольно просто разобраться и можно сразу приступать к поиску. Но если это вызывает у Вас трудности, вы можете ознакомиться с
                                    <a id="openGuide" class="link-success fw-bold fs-5" href="javascript:void(0);">инструкцией по применению.</a></li>
